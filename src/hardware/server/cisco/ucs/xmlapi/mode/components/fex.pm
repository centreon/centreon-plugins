...
package hardware::server::cisco::ucs::xmlapi::mode::components::fex;
...
sub load {
    my ($self) = @_;
    push @{$self->{request_classes}}, 'equipmentFex';
}

sub check {
    my ($self) = @_;

    $self->{output}->output_add(long_msg => 'Checking FEX');
    $self->{components}->{fex} = { name => 'fex', total => 0, skip => 0 };
    return if $self->check_filter(section => 'fex');

    foreach my $object (@{$self->{data}->{equipmentFex}}) {
        my $dn       = $object->{dn}        // 'unknown';
        my $presence = $object->{presence}  // 'unknown';
        my $state    = $object->{operState} // 'unknown';

        next if $self->check_filter(section => 'fex', instance => $dn);
        $self->{components}->{fex}->{total}++;

        $self->{output}->output_add(
            long_msg => sprintf("FEX '%s' presence is '%s' [operability: %s].", $dn, $presence, $state)
        );

        my $presence_threshold = $self->get_severity(
            section   => 'fex.presence',
        ...
        if (!$self->{output}->is_status(value => $presence_threshold, compare => 'ok', litteral => 1)) {
            $self->{output}->output_add(
                ...
                short_msg => sprintf("FEX '%s' presence is '%s'.", $dn, $presence)
            );
            next;
        }

        my $state_threshold = $self->get_severity(
            section   => 'fex.operability',
        ...
        if (!$self->{output}->is_status(value => $state_threshold, compare => 'ok', litteral => 1)) {
            $self->{output}->output_add(
                ...
                short_msg => sprintf("FEX '%s' operability is '%s'.", $dn, $state)
            );
        }
    }
}
