...
package hardware::server::cisco::ucs::xmlapi::mode::components::cpu;
...
sub load {
    my ($self) = @_;
    push @{$self->{request_classes}}, 'processorUnit';
}
...
sub check {
    ...
    foreach my $object (@{$self->{data}->{processorUnit}}) {
        my $dn       = $object->{dn}        // 'unknown';
        my $presence = $object->{presence}  // 'unknown';
        my $state    = $object->{operState} // 'unknown';

        next if $self->check_filter(section => 'cpu', instance => $dn);
        $self->{components}->{cpu}->{total}++;

        $self->{output}->output_add(
            long_msg => sprintf("CPU '%s' presence is '%s' [operability: %s].", $dn, $presence, $state)
        );

        my $presence_threshold = $self->get_severity(
            section   => 'cpu.presence',
        ...
        if (!$self->{output}->is_status(value => $presence_threshold, compare => 'ok', litteral => 1)) {
            $self->{output}->output_add(
                ...
                short_msg => sprintf("CPU '%s' presence is '%s'.", $dn, $presence)
            );
            next;
        }
        ...
        if (!$self->{output}->is_status(value => $state_threshold, compare => 'ok', litteral => 1)) {
            $self->{output}->output_add(
                ...
                short_msg => sprintf("CPU '%s' operability is '%s'.", $dn, $state)
            );
        }
    }
}
