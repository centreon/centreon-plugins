...
package hardware::server::cisco::ucs::xmlapi::mode::components::psu;
...
sub load {
    my ($self) = @_;
    push @{$self->{request_classes}}, 'equipmentPsu';
}
...
sub check {
    ...
    foreach my $object (@{$self->{data}->{equipmentPsu}}) {
        my $dn       = $object->{dn}        // 'unknown';
        my $presence = $object->{presence}  // 'unknown';
        my $state    = $object->{operState} // 'unknown';

        next if $self->check_filter(section => 'psu', instance => $dn);
        $self->{components}->{psu}->{total}++;

        $self->{output}->output_add(
            long_msg => sprintf("PSU '%s' presence is '%s' [operability: %s].", $dn, $presence, $state)
        );

        my $presence_threshold = $self->get_severity(
            section   => 'psu.presence',
        ...
        if (!$self->{output}->is_status(value => $presence_threshold, compare => 'ok', litteral => 1)) {
            $self->{output}->output_add(
                ...
                short_msg => sprintf("PSU '%s' presence is '%s'.", $dn, $presence)
            );
            next;
        }
        ...
        if (!$self->{output}->is_status(value => $state_threshold, compare => 'ok', litteral => 1)) {
            $self->{output}->output_add(
                ...
                short_msg => sprintf("PSU '%s' operability is '%s'.", $dn, $state)
            );
        }
    }
}
