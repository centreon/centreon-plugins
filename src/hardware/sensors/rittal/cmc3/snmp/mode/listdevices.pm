...
sub manage_selection {
    ...
    foreach my $oid (keys %{$snmp_result}) {
        ...
        if (defined($self->{option_results}->{filter_name}) && $self->{option_results}->{filter_name} ne '' &&
            ...
            $self->{output}->output_add(long_msg => "skipping  '" . $data->{dev_name} . "': no matching filter.");
            next;
        }

        if (defined($self->{option_results}->{filter_alias}) && $self->{option_results}->{filter_alias} ne '' &&
            $data->{dev_alias} !~ /$self->{option_results}->{filter_alias}/) {
            $self->{output}->output_add(long_msg => "skipping  '" . $data->{dev_alias} . "': no matching filter.");
            next;
        }
        ...
        $self->{devices}->{$instance} = {
            ...
            status_text => $data->{dev_status_text},
        };
    }
}
...
sub run {
    ...
    foreach my $instance (sort keys %{$self->{devices}}) {
        $self->{output}->output_add(long_msg => sprintf(
            "[index: %d] [name: '%s'] [alias: '%s'] [status: '%s'] [status text: '%s']",
            $self->{devices}->{$instance}->{index},
            $self->{devices}->{$instance}->{name},
            $self->{devices}->{$instance}->{alias},
            $self->{devices}->{$instance}->{status},
            $self->{devices}->{$instance}->{status_text},
        ));
    }
...
sub disco_format {
    ...
    $self->{output}->add_disco_format(elements => [ 'index', 'name', 'alias', 'status', 'status_text' ]);
}
...
sub disco_show {
    ...
    foreach my $instance (sort keys %{$self->{devices}}) {
        $self->{output}->add_disco_entry(
            index      => $self->{devices}->{$instance}->{index},
            name       => $self->{devices}->{$instance}->{name},
            alias      => $self->{devices}->{$instance}->{alias},
            status     => $self->{devices}->{$instance}->{status},
            statusText => $self->{devices}->{$instance}->{status_text},
        );
    }
}
