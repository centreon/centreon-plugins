...
package hardware::sensors::rittal::cmc3::snmp::mode::devicestatus;
...
sub set_counters {
    ...
    $self->{maps_counters_type} = [
        { name => 'device_status', type => 1, message_multiple => 'All devices are ok', skipped_code => { -10 => 1 } },
    ];
...
sub manage_selection {
    ...
    foreach my $oid (keys %{$snmp_result}) {
        ...
        if (defined($self->{option_results}->{index}) && $self->{option_results}->{index} ne '' &&
        ...
        if (defined($self->{option_results}->{filter_name}) && $self->{option_results}->{filter_name} ne '' &&
            ...
            $self->{output}->output_add(long_msg => "skipping  '" . $data->{name} . "': no matching filter.");
            next;
        }
        ...
        if (defined($self->{option_results}->{filter_alias}) && $self->{option_results}->{filter_alias} ne '' &&
            ...
            $self->{output}->output_add(long_msg => "skipping  '" . $data->{alias} . "': no matching filter.");
            next;
        }

        $self->{device_status}->{$instance} = {
            index  => $instance,
            name   => $data->{name},
            status => $data->{status},
            alias  => $data->{alias},
...
sub status_custom_output {
    ...
    return sprintf(
        "Device '%s' %s [alias: %s] [status text: %s]",
