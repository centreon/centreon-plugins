...
package os::windows::exporter::mode::interface;
...
sub custom_speed_calc {
    ...
    $self->{result_values}->{speed} = $options{new_datas}->{$self->{instance} . '_windows_net_current_bandwidth_bytes'};
    $self->{result_values}->{display} = $options{new_datas}->{$self->{instance} . '_display'};
    return 0;
}
    ...
    push @{$self->{maps_counters}->{interfaces}},
        {
            ...
            set => {
                key_values => [
                    { name => 'windows_net_bytes_received_total', diff => 1 },
                    { name => 'windows_net_current_bandwidth_bytes_in' },
                    { name => 'display' }
                ],
                closure_custom_calc => $self->can('custom_traffic_calc'),
                closure_custom_calc_extra_options => {
                    label_bytes => 'windows_net_bytes_received_total',
                    label_speed => 'windows_net_current_bandwidth_bytes_in',
                    label_output => 'In'
                },
        ...
        },
        {
            ...
            set => {
                key_values => [
                    { name => 'windows_net_bytes_sent_total', diff => 1 },
                    { name => 'windows_net_current_bandwidth_bytes_out' },
                    { name => 'display' }
                ],
                closure_custom_calc => $self->can('custom_traffic_calc'),
                closure_custom_calc_extra_options => {
                    label_bytes => 'windows_net_bytes_sent_total',
                    label_speed => 'windows_net_current_bandwidth_bytes_out',
                    label_output => 'Out'
                },
                closure_custom_output => $self->can('custom_traffic_output'),
                closure_custom_perfdata => $self->can('custom_traffic_perfdata'),
    ...
    push @{$self->{maps_counters}->{interfaces}},
        {
            ...
            set => {
                key_values => [
                    { name => 'windows_net_packets_received_discarded_total', diff => 1 },
                    { name => 'windows_net_packets_received_total', diff => 1 },
                    { name => 'display' }
                ],
                closure_custom_calc => $self->can('custom_errors_calc'),
                closure_custom_calc_extra_options => {
                    label_packets => 'windows_net_packets_received_discarded_total',
                    label_total => 'windows_net_packets_received_total',
        ...
        },
        {
            ...
            set => {
                key_values => [
                    { name => 'windows_net_packets_received_errors_total', diff => 1 },
                    { name => 'windows_net_packets_received_total', diff => 1 },
                    { name => 'display' }
                ],
                closure_custom_calc => $self->can('custom_errors_calc'),
                closure_custom_calc_extra_options => {
                    label_packets => 'windows_net_packets_received_errors_total',
                    label_total => 'windows_net_packets_received_total',
        ...
        },
        {
            ...
            set => {
                key_values => [
                    { name => 'windows_net_packets_outbound_discarded_total', diff => 1 },
                    { name => 'windows_net_packets_sent_total', diff => 1 },
                    { name => 'display' }
                ],
                closure_custom_calc => $self->can('custom_errors_calc'),
                closure_custom_calc_extra_options => {
                    label_packets => 'windows_net_packets_outbound_discarded_total',
                    label_total => 'windows_net_packets_sent_total',
        ...
        },
        {
            ...
            set => {
                key_values => [
                    { name => 'windows_net_packets_outbound_errors_total', diff => 1 },
                    { name => 'windows_net_packets_sent_total', diff => 1 },
                    { name => 'display' }
                ],
                closure_custom_calc => $self->can('custom_errors_calc'),
                closure_custom_calc_extra_options => {
                    label_packets => 'windows_net_packets_outbound_errors_total',
                    label_total => 'windows_net_packets_sent_total',
    ...
    push @{$self->{maps_counters}->{interfaces}},
        {
            ...
            set => {
                ...
                perfdatas => [
                    {
                        label => 'volume_in',
                        value => 'windows_net_bytes_received_total',
                        template => '%s',
                        unit => 'B',
                        min => 0,
                        label_extra_instance => 1,
        ...
        },
        {
            ...
            set => {
                ...
                perfdatas => [
                    {
                        label => 'volume_out',
                        value => 'windows_net_bytes_sent_total',
                        template => '%s',
                        unit => 'B',
                        min => 0,
                        label_extra_instance => 1,
...
sub skip_interface {
    ...
    return ($self->{checking} =~ /errors|traffic|volume/ ? 0 : 1);
}
...
sub check_options {
    ...
    if (!defined($self->{option_results}->{add_traffic}) && !defined($self->{option_results}->{add_errors}) &&
        !defined($self->{option_results}->{add_speed}) && !defined($self->{option_results}->{add_volume})) {
        $self->{option_results}->{add_traffic} = 1;
    }
    $self->{checking} = '';
    foreach (('add_traffic', 'add_errors', 'add_speed', 'add_volume')) {
...
sub manage_selection {
    ...
    my $raw_metrics = centreon::common::monitoring::openmetrics::scrape::parse(
        filter_metrics => 'windows_net_bytes|windows_net_packets|windows_net_current_bandwidth_bytes',
        %options
    );

    # windows_net_bytes_received_total{nic="net_prod_1"} 4.89182057e+08
    # windows_net_bytes_sent_total{nic="net_prod_1"} 3.78418824e+08
    # windows_net_bytes_total{nic="net_prod_1"} 8.67600881e+08
    # windows_net_packets_outbound_discarded_total{nic="net_prod_1"} 0
    # windows_net_packets_outbound_errors_total{nic="net_prod_1"} 0
    # windows_net_packets_received_discarded_total{nic="net_prod_1"} 0
    # windows_net_packets_received_errors_total{nic="net_prod_1"} 0
    # windows_net_packets_received_total{nic="net_prod_1"} 799087
    # windows_net_packets_received_unknown_total{nic="net_prod_1"} 0
    # windows_net_packets_sent_total{nic="net_prod_1"} 603419
    # windows_net_packets_total{nic="net_prod_1"} 1.402506e+06
    # windows_net_current_bandwidth_bytes{nic="net_prod_1"} 1.25e+09
    ...
    foreach my $metric (keys %{$raw_metrics}) {
        foreach my $data (@{$raw_metrics->{$metric}->{data}}) {
            ...
            $self->{interfaces}->{$data->{dimensions}->{nic}}->{$metric} = $data->{value};
            $self->{interfaces}->{$data->{dimensions}->{nic}}->{$metric} *= 8 if ($metric =~ /bytes/);
            $self->{interfaces}->{$data->{dimensions}->{nic}}->{display} = $data->{dimensions}->{nic};
    
            if ($metric =~ /windows_net_current_bandwidth_bytes/) {
                $self->{interfaces}->{$data->{dimensions}->{nic}}->{windows_net_current_bandwidth_bytes_in} = $self->{interfaces}->{$data->{dimensions}->{nic}}->{windows_net_current_bandwidth_bytes};
                $self->{interfaces}->{$data->{dimensions}->{nic}}->{windows_net_current_bandwidth_bytes_out} = $self->{interfaces}->{$data->{dimensions}->{nic}}->{windows_net_current_bandwidth_bytes};

                if (defined($self->{option_results}->{speed}) && $self->{option_results}->{speed} ne '') {
                    $self->{interfaces}->{$data->{dimensions}->{nic}}->{windows_net_current_bandwidth_bytes} = $self->{option_results}->{speed} * 1000000;
                    $self->{interfaces}->{$data->{dimensions}->{nic}}->{windows_net_current_bandwidth_bytes_in} = $self->{option_results}->{speed} * 1000000;
                    $self->{interfaces}->{$data->{dimensions}->{nic}}->{windows_net_current_bandwidth_bytes_out} = $self->{option_results}->{speed} * 1000000;
                }
                $self->{interfaces}->{$data->{dimensions}->{nic}}->{windows_net_current_bandwidth_bytes_in} = $self->{option_results}->{speed_in} * 1000000 if (defined($self->{option_results}->{speed_in}) && $self->{option_results}->{speed_in} ne '');
                $self->{interfaces}->{$data->{dimensions}->{nic}}->{windows_net_current_bandwidth_bytes_out} = $self->{option_results}->{speed_out} * 1000000 if (defined($self->{option_results}->{speed_out}) && $self->{option_results}->{speed_out} ne '');
            }
        }
    }
...
Check interfaces traffic, packets error/discard, speed and volume.

Uses metrics from https://github.com/prometheus-community/windows_exporter/blob/master/docs/collector.net.md.
