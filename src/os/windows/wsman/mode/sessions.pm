...
package os::windows::wsman::mode::sessions;
...
sub manage_selection {
    ...
    if (!defined($self->{option_results}->{command_path}) || $self->{option_results}->{command_path} eq '') {
        $qwinsta_cmd = $self->{option_results}->{command} . ' ' . $self->{option_results}->{command_options};
    } else {
         $qwinsta_cmd = $self->{option_results}->{command_path} . $self->{option_results}->{command}  . $self->{option_results}->{command_options};
    }
    $self->{result} = $self->{wsman}->execute_winshell_commands(commands => [ { label => 'qwinsta', value => $qwinsta_cmd } ]);

    my $datas = $self->read_qwinsta(stdout => $self->{result}->{qwinsta}->{stdout}, config => $config);
    my $counters = $self->read_qwinsta_counters(stdout => $self->{result}->{qwinsta}->{stdout}, config => $config);
