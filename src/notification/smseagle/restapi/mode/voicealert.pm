...
package notification::smseagle::restapi::mode::voicealert;
...
sub check_options {
    ...
    if (!centreon::plugins::misc::is_empty($self->{option_results}->{voice_id})
        && $self->{option_results}->{voice_id} !~ /^[0-9]+$/) {
        $self->{output}->add_option_msg(short_msg => "--voice-id must be a valid number");
        $self->{output}->option_exit();
    }
}
...
sub set_payload($$) {
    ...
    $body_obj->{voice_id} = int($self->{option_results}->{voice_id}) if defined($self->{option_results}->{voice_id});
...
sub run {
    ...
    $self->write_log("[INFO] new tts voice alert: To=$self->{option_results}->{to}, Body=$self->{option_results}->{orig_message_pattern}");
    my $endpoint = defined($self->{option_results}->{voice_id}) ? 'calls/tts_advanced' : 'calls/tts';
    ...
    if ($response == 1) {
        ...
        if ($http_status_code >= 400) {
            $msg = "[ERROR] Could not send tts voice alert $endpoint. Response: $api_response";
        ...
        my $msg = "Could not send tts voice alert $endpoint. $api_response";
        $self->write_log($msg);
        $self->{output}->add_option_msg(short_msg => $msg);
        $self->{output}->option_exit();
    }
