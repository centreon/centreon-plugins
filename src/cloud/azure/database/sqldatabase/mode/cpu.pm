...
package cloud::azure::database::sqldatabase::mode::cpu;
...
sub get_metrics_mapping {
    ...
    my $metrics_mapping = {
        'cpu_percent' => {
            'output' => 'CPU used percent',
            'label'  => 'usage-percent',
            'nlabel' => 'sqldatabase.cpu.usage.percentage',
            'unit'   => '%',
            'min'    => '0',
            'max'    => ''
        }
...
perl centreon_plugins.pl --plugin=cloud::azure::database::sqldatabase::plugin --mode=cpu --custommode=api
...
Warning threshold where '*' can be: 'usage-percent'.

=item B<--critical-*>

Critical threshold  where '*' can be: 'usage-percent'.
