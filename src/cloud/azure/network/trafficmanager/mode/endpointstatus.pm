...
package cloud::azure::network::trafficmanager::mode::endpointstatus;
...
sub check_options {
    ...
    if ($resource =~ /^\/subscriptions\/.*\/resourceGroups\/(.*)\/providers\/Microsoft\.Network\/trafficmanagerprofiles\/(.*)$/) {
        $resource_group = $1;
        $resource = $2;
    }
    ...
    $self->{az_resource_type} = 'trafficmanagerprofiles';
...
=item B<--warning-endpoint-status>

Warning threshold.

=item B<--critical-endpoint-status>
