...
package cloud::azure::compute::disk::mode::diskio;
...
sub check_options {
    ...
    if ($resource =~ /^\/subscriptions\/.*\/resourceGroups\/(.*)\/providers\/Microsoft\.compute\/disks\/(.*)\/provider\/microsoft.insights$/) {
        $resource_group = $1;
        $resource = $2;
    }
...
Check Azure Disk I/O volume and ops.
...
'disk-read-bytes', 'disk-write-bytes', 'disk-read-ops-persec', 'disk-write-ops-persec'
