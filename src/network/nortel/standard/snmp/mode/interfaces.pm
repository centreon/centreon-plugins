...
package network::nortel::standard::snmp::mode::interfaces;
...
sub load_status {
    ...
    $self->{oid_rcPortOperDuplex} = '.1.3.6.1.4.1.2272.1.4.10.1.1.13';
    $self->{oid_rcPortOperDuplex_mapping} = {
        0 => 'unknown', 1 => 'halfDuplex', 2 => 'fullDuplex',
    };

    $self->{snmp}->load(oids => [$self->{oid_rcPortOperDuplex}], instances => $self->{array_interface_selected});
}
...
sub add_result_status {
    ...
    if ($self->{int}->{ $options{instance} }->{duplexstatus} eq 'n/a') {
        $self->{int}->{ $options{instance} }->{duplexstatus} = defined($self->{results}->{ $self->{oid_rcPortOperDuplex} . '.' . $options{instance} }) ? $self->{oid_rcPortOperDuplex_mapping}->{ $self->{results}->{ $self->{oid_rcPortOperDuplex} . '.' . $options{instance} } } : 'n/a';
    }
}
