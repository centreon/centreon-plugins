...
sub set_counters {
    ...
    $self->{maps_counters}->{ont} = [
        ...
        {
            label => 'drop-packets', nlabel => 'ont.packets.drop.persecond', set => {
            key_values      => [ { name => 'drop_packets', per_second => 1 } ],
            output_template => 'dropped packets (per sec): %d',
            perfdatas       => [
                { label => 'drop_packets', template => '%d', min => 0, unit => 'packets/s' }
            ]
        }
        },
        {
...
sub manage_selection {
    ...
    $mapping = {
        ...
        drop_packets      => { oid => '.1.3.6.1.4.1.2011.6.128.1.1.4.23.1.5' },# hwGponOntStatisticUpDropPackts
    ...
    foreach (sort keys %{$self->{ont}}) {
        ...
        $self->{ont}->{$_}->{drop_packets} = $result->{drop_packets};
