...
package network::juniper::common::junos::netconf::mode::collection;
...
sub new {
    ...
    $self->{api_cache} = centreon::plugins::statefile->new(%options);
    return $self;
}
...
sub check_options {
    ...
    $self->{api_cache}->check_options(option_results => $self->{option_results});
}
...
sub validate_name {
    ...
    if (!defined($options{name})) {
        $self->{output}->add_option_msg(short_msg => "name attribute is missing in your api collection (path: $options{section})");
        $self->{output}->option_exit();
    }
    if ($options{name} !~ /^[a-zA-Z0-9]+$/) {
        $self->{output}->add_option_msg(short_msg => "name attribute in your api collection (path: $options{section}) is incorrect: " . $options{name});
        $self->{output}->option_exit();
    }
}
...
sub parse_txt {
    ...
    if (!defined($options{conf}->{name}) || $options{conf}->{name} eq '') {
        $self->{output}->add_option_msg(short_msg => "name attribute is missing [api > commands > $options{name}]");
        $self->{output}->option_exit();
    }
    if (!defined($options{conf}->{re}) || $options{conf}->{re} eq '') {
        $self->{output}->add_option_msg(short_msg => "re attribute is missing [api > commands > $options{name} > $options{conf}->{name}]");
        $self->{output}->option_exit();
    }
    if (!defined($options{conf}->{entries})) {
        $self->{output}->add_option_msg(short_msg => "entries section is missing [api > commands > $options{name} > $options{conf}->{name}]");
        $self->{output}->option_exit();
    }
    foreach (@{$options{conf}->{entries}}) {
        if (!defined($_->{id})) {
            $self->{output}->add_option_msg(short_msg => "id attribute is missing or wrong [api > commands > $options{name} > $options{conf}->{name}]");
            $self->{output}->option_exit();
        }
        if (!defined($_->{offset}) || $_->{offset} !~ /^(?:[0-9]+)$/) {
            $self->{output}->add_option_msg(short_msg => "offset attribute is missing or wrong [api > commands > $options{name} > $options{conf}->{name}]");
            $self->{output}->option_exit();
        }
    }
        ...
        foreach (@entries) {
            ...
            if (defined($_->{map}) && $_->{map} ne '') {
                if (!defined($self->{config}->{mapping}) || !defined($self->{config}->{mapping}->{ $_->{map} })) {
                    $self->{output}->add_option_msg(short_msg => "unknown map attribute [api > commands > $options{name} > $options{conf}->{name}]: $_->{map}");
                    $self->{output}->option_exit();
                }
            ...
            if (defined($_->{sampling}) && $_->{sampling} == 1) {
                $self->{api_collected_sampling}->{tables}->{$name} = {}
                    if (!defined($self->{api_collected_sampling}->{tables}->{$name}));
                $self->{api_collected_sampling}->{tables}->{$name}->{$instance}->{ $_->{id} } = $value;
            }
        }

        $self->{api_collected}->{tables}->{$name}->{$instance} = $entry;
...
sub parse_structure {
    ...
    if (!defined($options{conf}->{path}) || $options{conf}->{path} eq '') {
        $self->{output}->add_option_msg(short_msg => "path attribute is missing [api > commands > $options{name} > $options{conf}->{name}]");
        $self->{output}->option_exit();
    }
    if (!defined($options{conf}->{entries})) {
        $self->{output}->add_option_msg(short_msg => "entries section is missing [api > commands > $options{name} > $options{conf}->{name}]");
    ...
    foreach my $value (@values) {
        ...
        foreach (@{$options{conf}->{entries}}) {
            ...
            if (defined($_->{sampling}) && $_->{sampling} == 1) {
                $self->{api_collected_sampling}->{tables}->{$name} = {}
                    if (!defined($self->{api_collected_sampling}->{tables}->{$name}));
                $self->{api_collected_sampling}->{tables}->{$name}->{$instance}->{ $_->{id} } = $entry->{ $_->{id} };
            }
        }
...
sub collect_api_commands {
    ...
    return if (!defined($self->{config}->{api}->{commands}));
    foreach my $command (@{$self->{config}->{api}->{commands}}) {
        $self->validate_name(name => $command->{name}, section => "[api > commands]");
        if (!defined($command->{command}) || $command->{command} eq '') {
            $self->{output}->add_option_msg(short_msg => "command attribute is missing [api > commands > $command->{name}]");
            $self->{output}->option_exit();
        }

        my $content = $options{custom}->get_command_raw_result(command => $command->{command});

        my $local = {};
        if (defined($command->{parse})) {
            foreach my $conf (@{$command->{parse}}) {
                my $lentries = {};
                if ($command->{rtype} eq 'txt') {
...
sub use_api_cache {
    ...
    return 0 if ($self->is_api_cache_enabled() == 0);
...
sub save_api_cache {
    ...
    return 0 if ($self->is_api_cache_enabled() == 0);
    $self->{api_cache}->write(data => { api_collected => $self->{api_collected} });
}

sub collect_api_sampling {
    my ($self, %options) = @_;

    return if ($self->{api_collected}->{sampling} == 0);

    my $has_cache_file = $self->{api_cache}->read(
        statefile => 'cache_juniper_collection_sampling_' . md5_hex($self->{option_results}->{config})
    );
    my $api_collected_sampling_old = $self->{api_cache}->get(name => 'api_collected_sampling');
    ...
    if (!defined($self->{api_collected_sampling})) {
        $self->{api_collected_sampling} = $api_collected_sampling_old;
    }

    my $delta_time;
    if (defined($api_collected_sampling_old->{epoch})) {
        $delta_time = $self->{api_collected_sampling}->{epoch} - $api_collected_sampling_old->{epoch};
        $delta_time = 1 if ($delta_time <= 0);
    }

    foreach my $tbl_name (keys %{$self->{api_collected_sampling}->{tables}}) {
        foreach my $instance (keys %{$self->{api_collected_sampling}->{tables}->{$tbl_name}}) {
            foreach my $attr (keys %{$self->{api_collected_sampling}->{tables}->{$tbl_name}->{$instance}}) {
                next if (
                    !defined($api_collected_sampling_old->{tables}->{$tbl_name}) ||
                    !defined($api_collected_sampling_old->{tables}->{$tbl_name}->{$instance}) ||
                    !defined($api_collected_sampling_old->{tables}->{$tbl_name}->{$instance}->{$attr}) ||
                    $api_collected_sampling_old->{tables}->{$tbl_name}->{$instance}->{$attr} !~ /\d/
                );
                my $old = $api_collected_sampling_old->{tables}->{$tbl_name}->{$instance}->{$attr};
                my $diff = $self->{api_collected_sampling}->{tables}->{$tbl_name}->{$instance}->{$attr} - $old;
                my $diff_counter = $diff;
                $diff_counter = $self->{api_collected_sampling}->{tables}->{$tbl_name}->{$instance}->{$attr} if ($diff_counter < 0);

                $self->{api_collected}->{tables}->{$tbl_name}->{$instance}->{ $attr . 'Diff' } = $diff;
                $self->{api_collected}->{tables}->{$tbl_name}->{$instance}->{ $attr . 'DiffCounter' } = $diff_counter;
                if (defined($delta_time)) {
                    $self->{api_collected}->{tables}->{$tbl_name}->{$instance}->{ $attr . 'PerSeconds' } = $diff_counter / $delta_time;
                    $self->{api_collected}->{tables}->{$tbl_name}->{$instance}->{ $attr . 'PerMinutes' } = $diff_counter / $delta_time / 60;
                }
            }
        }
    }

    $self->{api_cache}->write(data => { api_collected_sampling => $self->{api_collected_sampling} });
}
...
sub display_variables {
    ...
    foreach my $tbl_name (keys %{$self->{api_collected}->{tables}}) {
        my $expr = 'api.tables.' . $tbl_name;
        foreach my $instance (keys %{$self->{api_collected}->{tables}->{$tbl_name}}) {
            foreach my $attr (keys %{$self->{api_collected}->{tables}->{$tbl_name}->{$instance}}) {
...
sub collect_api {
    ...
    if (!defined($self->{config}->{api})) {
        $self->{output}->add_option_msg(short_msg => 'please set api config');
        $self->{output}->option_exit();
    }

    $self->add_builtin(name => 'currentTime', value => time());
    if ($self->use_api_cache(custom => $options{custom}) == 0) {
        $self->{api_collected_sampling} = { tables => {}, epoch => time() };
        $self->{api_collected} = { tables => {}, epoch => time(), sampling => 0 };

        $self->collect_api_commands(custom => $options{custom});

        $self->{api_collected}->{sampling} = 1 if (
            scalar(keys(%{$self->{api_collected_sampling}->{tables}})) > 0
        );
        $self->save_api_cache();
    }

    $self->collect_api_sampling(custom => $options{custom});
...
sub exist_table_name {
    ...
    return 1 if (defined($self->{api_collected}->{tables}->{ $options{name} }));
    return 0;
}
...
sub get_table {
    ...
    if (!defined($self->{api_collected}->{tables}->{ $options{table} })) {
        $self->{output}->add_option_msg(short_msg => "Table '" . $options{table} . "' not found in ('" . join("', '", keys(%{$self->{api_collected}->{tables}})) . "')", debug => 1);
        return undef;
    }
    return $self->{api_collected}->{tables}->{ $options{table} };
}
...
sub get_table_attribute_value {
    ...
    return undef if (
        !defined($self->{api_collected}->{tables}->{ $options{table} }) ||
        !defined($self->{api_collected}->{tables}->{ $options{table} }->{ $options{instance} }) ||
        !defined($self->{api_collected}->{tables}->{ $options{table} }->{ $options{instance} }->{ $options{attribute} })
    );
    return $self->{api_collected}->{tables}->{ $options{table} }->{ $options{instance} }->{ $options{attribute} };
}

sub set_table_attribute_value {
    my ($self, %options) = @_;

    $self->{api_collected}->{tables}->{ $options{table} } = {}
        if (!defined($self->{api_collected}->{tables}->{ $options{table} }));
    $self->{api_collected}->{tables}->{ $options{table} } = {}
        if (!defined($self->{api_collected}->{tables}->{ $options{table} }->{ $options{instance} }));
    $self->{api_collected}->{tables}->{ $options{table} }->{ $options{instance} }->{ $options{attribute} } = $options{value};
}
...
managed variables:
    ...
    %(api.tables.centreonTest.[1])
    %(api.tables.centreonTest.[1].plop)
    %(api.tables.centreonTest.[%(mytable.instance)]
    %(api.tables.centreonTest.[%(api.tables.centreonTest.[%(mytable.instance)].name)]
    %(test2)
    %(mytable.test)
...
result:
    - type:
        0=%(test) (label)
        2=%(api.tables.centreonTest)
        3=%(api.tables.centreonTest.[2])
        4=%(api.tables.centreonTest.[2].attrname)
=cut
sub parse_api_tables {
    my ($self, %options) = @_;
    ...
    if (!$self->exist_table_name(name => $table_label)) {
        my @names = keys %{$self->{api_collected}->{tables}};
    ...
    if (!defined($options{chars}->[$end + 1]) || $options{chars}->[$end + 1] ne '[') {
        $self->{output}->add_option_msg(short_msg => $self->{current_section} . " special variable api.tables character '[' mandatory");
        $self->{output}->option_exit();
    }
    ...
    if (!defined($options{chars}->[$end + 1]) ||
        $options{chars}->[$end + 1] !~ /[.)]/) {
        $self->{output}->add_option_msg(short_msg => $self->{current_section} . ' special variable api.tables character [.)] missing');
        $self->{output}->option_exit();
    }
...
sub parse_api_type {
    ...
    if ($self->strcmp(chars => $options{chars}, start => $options{start}, test => 'tables.')) {
        return $self->parse_api_tables(chars => $options{chars}, start => $options{start} + 7);
    } else {
        $self->{output}->add_option_msg(short_msg => $self->{current_section} . ' special variable api not followed by tables');
        $self->{output}->option_exit();
    }
}
...
sub parse_special_variable {
    ...
    if ($self->strcmp(chars => $options{chars}, start => $start + 2, test => 'api.')) {
        my $parse = $self->parse_api_type(chars => $options{chars}, start => $start + 2 + 4);
        $result = { %$parse, %$result };
    } else {
...
sub exec_func_assign {
    ...
    #{
    #   "type": "assign",
    #   "save": "%(api.tables.test)",
    #   "expression": "'%(api.tables.test)' . 'toto'"
    #}
...
sub add_selection_loop {
    ...
    foreach (@{$self->{config}->{selection_loop}}) {
        ...
        next if (!defined($self->{api_collected}->{tables}->{ $result->{table} }));

        foreach my $instance (keys %{$self->{api_collected}->{tables}->{ $result->{table} }}) {
            $self->{expand} = $self->set_constants();
            $self->set_builtin();
            $self->set_local_variables();
            $self->{expand}->{ $result->{table} . '.instance' } = $instance;

            foreach my $label (keys %{$self->{api_collected}->{tables}->{ $result->{table} }->{$instance}}) {
                $self->{expand}->{ $result->{table} . '.' . $label } =
                    $self->{api_collected}->{tables}->{ $result->{table} }->{$instance}->{$label};
            }
...
sub disco_show {
    ...
    $self->collect_api(custom => $options{custom});
