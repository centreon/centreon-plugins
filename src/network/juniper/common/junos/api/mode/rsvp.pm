...
sub set_counters {
    ...
    $self->{maps_counters}->{rsvp} = [
        ...
        { label => 'rsvp-session-lsp-traffic', nlabel => 'rsvp.session.lsp.traffic.bytes', set => {
                key_values => [ { name => 'lspBytes' }, { name => 'type' }, { name => 'name' }, { name => 'srcAddress' }, { name => 'dstAddress' } ],
                output_template => 'traffic: %s%s',
