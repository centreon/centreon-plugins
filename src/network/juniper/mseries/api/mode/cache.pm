...
package network::juniper::mseries::api::mode::cache;
...
sub check_options {
    ...
    if (!defined($self->{option_results}->{commands}) || $self->{option_results}->{commands} eq '') {
        $self->{option_results}->{commands} = 'cpu,disk,hardware,interface,memory';
    }
...
For which modes the cache file is done (default: 'cpu,disk,hardware,interface,memory').
