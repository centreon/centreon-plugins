...
package network::vectra::restapi::mode::uptime;
...
sub manage_selection {
    ...
    my $system = $options{custom}->request_api(endpoint => '/health/system');

    my ($day, $hour, $min) = (0, 0, 0);
    $day = $1 if ($system->{system}->{uptime} =~ /(\d+)\s+day/);
    $hour = $1 if ($system->{system}->{uptime} =~ /(\d+)\s+hour/);
    $min = $1 if ($system->{system}->{uptime} =~ /(\d+)\s+minute/);

    $self->{global} = {
        uptime => ($day * 86400) + ($hour * 3600) + ($min * 60)
    };

    if (defined($self->{option_results}->{add_system_info})) {
        $self->{output}->output_add(short_msg => sprintf(
                'model: %s, firmware: %s',
                $system->{system}->{version}->{model},
                $system->{system}->{version}->{vectra_version}
            )
        );
    }
}
...
Display model and firmware informations
