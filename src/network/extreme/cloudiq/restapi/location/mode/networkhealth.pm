...
package network::extreme::cloudiq::restapi::location::mode::networkhealth;
...
sub custom_usage_output {
    ...
    my ($usage_value, $usage_unit) = $self->{perfdata}->change_bytes(value => $self->{result_values}->{network_usage});

    return sprintf('Network usage: %s%s', $usage_value . " " . $usage_unit);
}
...
sub set_counters {
    ...
    $self->{maps_counters}->{global} = [
        ...
        {
            label  => 'internet-availability-score',
            nlabel => 'internet.availability.score',
            set    => {
                key_values      => [ { name => 'internet_availability_score' } ],
                output_template => 'internet availability score: %d',
                perfdatas       => [
                    { template => '%d', min => 0, max => 100 }
                ]
            }
        },
        {
            ...
            nlabel => 'internet.performance',
            set    => {
                key_values      => [ { name => 'internet_performance' } ],
                output_template => 'internet performance: %d ms',
                perfdatas       => [
                    { template => '%d', min => 0, unit => 'ms', cast_int => 1 }
                ]
            }
        },
        {
            label  => 'network-usage',
            nlabel => 'network.usage',
            set    => {
                key_values            => [ { name => 'network_usage' } ],
                closure_custom_output => $self->can('custom_usage_output'),
                perfdatas             => [
                    { template => '%d', min => 0, unit => 'B', cast_int => 1 }
                ]
            }
        }
    ];
...
sub manage_selection {
    ...
    $endpoint = sprintf(
        "/network-scorecard/networkHealth/%s?startTime=%s&endTime=%s",
        $location_id,
        $start,
        $end
    );
    ...
    $self->{global} = {
        ...
        internet_availability_score => $scores->{internet_availability_score},
        internet_performance        => $scores->{internet_performance},
        network_usage               => $scores->{network_usage} * 1024 * 1024,
...
Check Extreme Cloud IQ location network health scores.
...
=item B<--warning-internet-availability-score>

Warning threshold for internet availability score. (0-100)

=item B<--critical-internet-availability-score>

Critical threshold for internet availability score. (0-100)

=item B<--warning-internet-performance>

Warning threshold for internet performance. (ms)

=item B<--critical-internet-performance>

Critical threshold for internet performance. (ms)

=item B<--warning-network-usage>

Warning threshold for network usage. (B)

=item B<--critical-network-usage>

Critical threshold for network usage. (B)
