...
package network::extreme::cloudiq::restapi::location::mode::serviceshealth;
...
sub set_counters {
    ...
    $self->{maps_counters}->{global} = [
        ...
        { label => 'network-service-score', nlabel => 'network.service.score', set => {
            key_values      => [ { name => 'network_services_score' } ],
            output_template => 'network service score: %d',
            perfdatas       => [
                { template => '%d', min => 0, max => 100 }
            ]
        }
...
sub manage_selection {
    ...
    $endpoint = sprintf(
        "/network-scorecard/servicesHealth/%s?startTime=%s&endTime=%s",
        $location_id,
        $start,
        $end
    );
    ...
    $self->{global} = {
        ...
        network_services_score => $scores->{network_services_score},
...
Check Extreme Cloud IQ location client services health scores.
...
=item B<--warning-network-services-score>

Warning threshold for network services score. (0-100)

=item B<--critical-network-services-score>

Critical threshold for network services score. (0-100)
