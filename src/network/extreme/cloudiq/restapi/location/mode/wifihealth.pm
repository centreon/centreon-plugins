...
package network::extreme::cloudiq::restapi::location::mode::wifihealth;
...
sub set_counters {
    ...
    $self->{maps_counters}->{global} = [
        { label => 'overall-score', nlabel => 'overall.score', set => {
        ...
        { label => 'snr-score', nlabel => 'snr.score', set => {
            key_values      => [ { name => 'snr_score' } ],
            output_template => 'Signal to noise radio score: %d',
            perfdatas       => [
                { template => '%d', min => 0, max => 100 }
            ]
        }
        },
        { label => 'channel-utilization-score', nlabel => 'channel.utilization.score', set => {
            key_values      => [ { name => 'channel_utilization_score' } ],
            output_template => 'channel utilization score: %d',
            perfdatas       => [
                { template => '%d', min => 0, max => 100 }
            ]
        }
        },
        { label => 'association-radio-score', nlabel => 'association.radio.score', set => {
            key_values      => [ { name => 'association_per_radio_score' } ],
            output_template => 'association per radio score: %d',
            perfdatas       => [
                { template => '%d', min => 0 }
            ]
        }
...
sub manage_selection {
    ...
    $endpoint = sprintf(
        "/network-scorecard/wifiHealth/%s?startTime=%s&endTime=%s",
        $location_id,
        $start,
        $end
    );
    ...
    $self->{global} = {
        ...
        channel_utilization_score   => $scores->{channel_utilization_score},
        association_per_radio_score => $scores->{association_per_radio_score},
...
Check Extreme Cloud IQ location wifi health scores.
...
=item B<--warning-snr-score>

Warning threshold for SNR score. (0-100)

=item B<--critical-snr-score>

Critical threshold for snr (signal to noise radio) score. (0-100)

=item B<--warning-channel-utilization-score>

Warning threshold for channel utilization score. (0-100)

=item B<--critical-channel-utilization-score>

Critical threshold for channel utilization score. (0-100)

=item B<--warning-association-per-radio-score>

Warning threshold for association per radio score. (0-100)

=item B<--critical-association-per-radio-score>

Critical threshold for association per radio score. (0-100)
