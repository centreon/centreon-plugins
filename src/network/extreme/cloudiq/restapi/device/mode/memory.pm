...
package network::extreme::cloudiq::restapi::device::mode::memory;
...
sub set_counters {
    ...
    $self->{maps_counters_type} = [
        { name => 'memory_avg', type => 0, cb_prefix_output => 'prefix_memory_avg_output' }
    ];

    $self->{maps_counters}->{memory_avg} = [
        {
            label  => 'average',
            nlabel => 'memory.utilization.percentage',
            set    => {
...
sub manage_selection {
    ...
    $endpoint = sprintf(
        "/devices/%s/history/cpu-mem?startTime=%s&endTime=%s",
        $device_id,
        $start,
        $end
    );
    ...
    if (scalar(@{$values}) <= 0) {
        $self->{output}->add_option_msg(short_msg => 'No data found. Please check time-interval');
        $self->{output}->option_exit();
    }
    ...
    foreach my $value (@{$values}) {
        $avg_total += $value->{average_memory};
        $cnt++;
    }
...
Check Extreme Cloud IQ device average memory.
...
Warning threshold for memory average.

=item B<--critical-average>

Critical threshold for memory average.
