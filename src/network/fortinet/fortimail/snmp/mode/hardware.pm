...
sub check {
    ...
    foreach my $oid ($self->{snmp}->oid_lex_sort(keys %{$self->{results}->{$oid_fgHwSensorEntry}})) {
        ...
        my $result = $self->{snmp}->map_instance(mapping =>
            $mapping,
            results                                      =>
                $self->{results}->{$oid_fgHwSensorEntry},
            instance                                     =>
                $instance);
        ...
        my $exit = $self->get_severity(section =>
            'sensors',
            name                               =>
                $name,
            value                              =>
                $result->{fgHwSensorEntAlarmStatus});
        ...
        my ($exit2, $warn, $crit, $checked) = $self->get_severity_numeric(section =>
            'sensors',
            instance                                                              =>
                $instance,
            name                                                                  =>
