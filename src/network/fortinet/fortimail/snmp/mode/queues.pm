...
sub set_counters {
    ...
    $self->{maps_counters}->{queue} = [
        { label => 'count', nlabel => 'queue.mail.count', set => {
            key_values      => [ { name => 'count' }, { name => 'display' } ],
            output_template => 'Mails: %s',
            perfdatas       => [
                { label => 'mail-count', template => '%d',
                    min => 0, label_extra_instance => 1, instance_use => 'display' }
            ]
        }
        },
        { label => 'size', nlabel => 'queue.mail.size', set => {
            key_values      => [ { name => 'size' }, { name => 'display' } ],
            output_template => 'Size : %s KB',
            perfdatas       => [
                { label => 'mail-size', template => '%d', unit => 'KB',
                    min => 0, label_extra_instance => 1, instance_use => 'display' }
            ]
        }
        }
...
sub manage_selection {
    ...
    foreach my $oid (keys %{$snmp_result->{$oid_fmlMailQueueName}}) {
        ...
        if (defined($self->{option_results}->{filter_name}) && $self->{option_results}->{filter_name} ne '' &&
            ...
            $self->{output}->output_add(
                long_msg => "skipping  '" . $snmp_result->{$oid_fmlMailQueueName}->{$oid} . "': no matching filter.");
            next;
        }
        ...
        $self->{queue}->{$instance} = {
            ...
            size    => $result->{'fmlMailQueueMailSize'},
        };
    }
    ...
    $self->{cache_name} = "fortinet_fortimail_" . $self->{mode} . '_'
        . $options{snmp}->get_hostname() . '_' . $options{snmp}->get_port() . '_' .
