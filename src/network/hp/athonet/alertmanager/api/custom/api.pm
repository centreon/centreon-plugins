...
sub check_options {
    ...
    $self->{url_path} = (defined($self->{option_results}->{url_path})) ? $self->{option_results}->{url_path} : '/core/alertmanager/api/v2';
...
sub get_access_token {
    ...
    my $has_cache_file = $self->{cache}->read(statefile => 'hp_athonet_alertmanager_' . md5_hex($self->get_connection_info() . '_' . $self->{api_username}));
