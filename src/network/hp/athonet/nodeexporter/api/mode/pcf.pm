...
package network::hp::athonet::nodeexporter::api::mode::pcf;
...
sub set_counters {
    ...
    $self->{maps_counters}->{global} = [
        ...
        { label => 'sessions-n5', nlabel => 'pcf.sessions.n5.count', set => {
                key_values => [ { name => 'pcf_n5_sessions' } ],
                output_template => 'N5 sessions: %s',
                perfdatas => [
                    { template => '%s', min => 0 }
                ]
            }
...
sub manage_selection {
    ...
    my $response = $options{custom}->query(queries => ['pcf_n7_sessions']);
    $self->{global}->{pcf_n7_sessions} = $response->[0]->{value}->[1];

    $response = $options{custom}->query(queries => ['pcf_n5_sessions']);
    $self->{global}->{pcf_n5_sessions} = $response->[0]->{value}->[1];

    my $map_interface_status = { 1 => 'up', 0 => 'down' };

    $response = $options{custom}->query(queries => ['diameter_peer_status{target_type="pcf"}']);
    $self->{diameters} = {};
    my $id = 0;
    foreach (@$response) {
        $self->{diameters}->{$id} = {
