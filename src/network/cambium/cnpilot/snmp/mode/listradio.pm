...
package network::cambium::cnpilot::snmp::mode::listradio;
...
sub run {
    ...
    foreach my $oid_path (sort keys %$results) {
        $self->{output}->output_add(
            long_msg => sprintf(
                '[name: %s] [radio channel: %s] [band type: %s]',
...
sub disco_format {
    ...
    $self->{output}->add_disco_format(elements => ['name','radio_channel','band_type']);
}
