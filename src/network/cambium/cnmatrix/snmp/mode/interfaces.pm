...
package network::cambium::cnmatrix::snmp::mode::interfaces;
...
sub set_counters_errors {
    ...
    push @{$self->{maps_counters}->{int}},
        ...
        },
        {
            ...
            filter => 'add_optical',
            nlabel => 'interface.module.temperature.celsius',
            set    =>
                {
...
sub new {
    ...
    $options{options}->add_options(arguments => { 'add-optical' => { name => 'add_optical' } });
...
sub reload_cache_custom {
    ...
    my $oid_cnTransceiverVendorName = '.1.3.6.1.4.1.2076.81.18.1.1.11.1.5';
    my $snmp_result = $self->{snmp}->get_table(oid => $oid_cnTransceiverVendorName);

    foreach (keys %$snmp_result) {
        next if (!/^$oid_cnTransceiverVendorName\.(\d+)$/);
        $options{datas}->{optical_ports}->{$1} = $snmp_result->{$_};
    }
}

my $mapping_optical = {
    output_power       => { oid => '.1.3.6.1.4.1.2076.81.18.1.1.11.1.14' },# cnTransceiverTxPower
    input_power        => { oid => '.1.3.6.1.4.1.2076.81.18.1.1.11.1.15' },# cnTransceiverRxPower
    module_temperature => { oid => '.1.3.6.1.4.1.2076.81.18.1.1.11.1.11' },# cnTransceiverTemperature
    voltage            => { oid => '.1.3.6.1.4.1.2076.81.18.1.1.11.1.12' },# cnTransceiverVoltage
    bias_current       => { oid => '.1.3.6.1.4.1.2076.81.18.1.1.11.1.13' }# cnTransceiverTxBias
};
...
Display traffic perfdata to be compatible with C<nagvis> widget.
