...
sub check_options {
    ...
    if (($self->{perfdata}->threshold_validate(
        label => 'warning', value => $self->{option_results}->{warning})) == 0
    ) {
    ...
    if (($self->{perfdata}->threshold_validate(
        label => 'critical', value => $self->{option_results}->{critical})) == 0
    ) {
        $self->{output}->add_option_msg(
...
sub run {
    ...
    if (defined($old_datas->{old_timestamp})) {
        ...
        $self->{output}->output_add(
            severity  => $exit,
            short_msg => sprintf("Packets Rejected : %.2f /s [%i packets]", $rejected_per_sec, $rejected)
        );

        $self->{output}->perfdata_add(
            label    => 'rejected_packets_per_sec',
