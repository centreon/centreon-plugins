...
sub ap_long_output {
    ...
    return sprintf(
        'checking access point %s - BSS ID %s - ESS ID %s ',
...
sub set_counters {
    ...
    $self->{maps_counters}->{health} = [
        ...
        { label => 'channel', nlabel => 'vap.channel.count', set => {
        ...
        { label => 'extended-channel', nlabel => 'vap.extended.channel.count', set => {
        ...
        { label => 'number-stations', nlabel => 'vap.number.stations.count', set => {
        ...
        { label => 'traffic-in', nlabel => 'vap.traffic.in.bitspersecond', set => {
        ...
        { label => 'traffic-out', nlabel => 'vap.traffic.out.bitspersecond', set => {
        ...
        { label => 'dropped-in', nlabel => 'vap.packets.in.dropped.percentage', set => {
            key_values                        =>
        ...
        { label => 'dropped-out', nlabel => 'vap.packets.out.dropped.percentage', set => {
            key_values                        =>
        ...
        { label => 'error-in', nlabel => 'vap.packets.in.error.percentage', set => {
            key_values                        =>
        ...
        { label => 'error-out', nlabel => 'vap.packets.out.error.percentage', set => {
            key_values                        =>
        ...
        { label => 'output-power', nlabel => 'vap.output.power.dbm', set => {
            key_values            =>
...
my $mapping_stat = {
    ...
    txUp        => { oid => '.1.3.6.1.4.1.41112.1.6.1.2.1.22', map => $up_status },# unifiVapUp
...
sub manage_selection {
    ...
    foreach (sort keys %{$self->{vap}}) {
        ...
        $self->{vap}->{$_}->{bss_id} = $result->{bssId};
