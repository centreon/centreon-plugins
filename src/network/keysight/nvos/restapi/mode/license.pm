...
sub custom_status_output {
    ...
    return 'License expiration status: ' . $self->{result_values}->{status}. "\n". $self->{result_values}->{info};
}
...
sub manage_selection {
    ...
    $self->{global} = {
        status => $result->{subsystem_alarms}->{subsystem_alarms}->{LicenseExpirationStatus}->{state},
        info => $result->{subsystem_alarms}->{subsystem_alarms}->{LicenseExpirationStatus}->{info}[0]
    };
}
