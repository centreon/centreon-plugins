...
package network::keysight::nvos::restapi::mode::uptime;
...
sub manage_selection {
    ...
    my $result = $options{custom}->request_api(
        endpoint => '/api/system',
        get_param => ['properties=ready_time']
    );
    if (!defined($result->{ready_time})) {
        $self->{output}->add_option_msg(short_msg => "Cannot find uptime information");
        $self->{output}->option_exit();
    }

    $self->{global} = { uptime => time() - ($result->{ready_time} / 1000) };
}
