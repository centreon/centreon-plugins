...
package apps::saas::apivideo::restapi::custom::api;
...
sub new {
    ...
    if (!defined($options{noptions})) {
        $options{options}->add_options(arguments => {
            'hostname:s'             => { name => 'hostname', default => 'ws.api.video' },
...
sub check_options {
    ...
    if (is_empty($self->{option_results}->{api_key})) {
...
sub get_access_token {
    ...
    my $has_cache_file = $self->{cache}->read(statefile => 'apivideo_connect_' . $self->get_connection_info());
    ...
    my $md5_secret = md5_hex($self->{option_results}->{api_key});

    if (defined($refresh_token) && !defined($access_token)) {
        my $json_request = {
            refreshToken => $refresh_token
        };
        my $encoded = json_encode(
            $json_request,
    ...
    if ($has_cache_file == 0 ||
        ...
        my $json_request = {
            apiKey => $self->{option_results}->{api_key}
        };
        my $encoded;
        eval {
...
api.video Rest API custom mode.
...
Prometheus hostname (default: C<'ws.api.video'>).
