...
sub custom_usage_perfdata {
    ...
    if ($self->{instance_mode}->{option_results}->{units} eq '%') {
        $total_options{total} = $self->{result_values}->{node_filesystem_size_bytes};
...
sub custom_usage_output {
    ...
    my ($total_size_value, $total_size_unit) = $self->{perfdata}->change_bytes(value => $self->{result_values}->{node_filesystem_size_bytes});
    my ($total_used_value, $total_used_unit) = $self->{perfdata}->change_bytes(value => $self->{result_values}->{used});
    my ($total_free_value, $total_free_unit) = $self->{perfdata}->change_bytes(value => $self->{result_values}->{node_filesystem_free_bytes});
    return sprintf(
...
sub custom_usage_calc {
    ...
    if ($self->{result_values}->{prct_used} > 100) {
        $self->{result_values}->{node_filesystem_free_bytes} = 0;
...
sub set_counters {
    ...
    $self->{maps_counters}->{node_storage} = [
        { label => 'usage', set => {
                key_values => [ { name => 'node_filesystem_free_bytes' }, { name => 'node_filesystem_size_bytes' }, { name => 'display' } ],
