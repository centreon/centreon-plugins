...
package apps::controlm::restapi::custom::api;
...
sub new {
    ...
    $options{options}->add_help(package => __PACKAGE__, sections => 'AUTOMATION API OPTIONS', once => 1);
...
sub get_token {
    ...
    my $has_cache_file = $self->{cache}->read(statefile => 'controlm_' . md5_hex($self->get_connection_info() . '_' . $self->{api_username}));
