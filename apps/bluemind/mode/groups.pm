...
package apps::bluemind::mode::groups;
...
sub run {
    ...
    foreach my $item(@$webcontent2)
    {
      ...
      $hash{$group}=$item->{'value'}{'memberCount'};

      $i=$i+1;


      $self->{output}->perfdata_add(label    => $group,
                                    value    => $item->{'value'}{'memberCount'}.' '.'member(s)');
...
Check groups from Bluemind server.
