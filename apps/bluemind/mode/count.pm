...
package apps::bluemind::mode::count;
...
sub run {
    ...
    foreach my $item(@$webcontent2)
    {
     ...
     $self->{output}->output_add(severity  => 'OK',
                                 short_msg => $i.' '.'mailboxes on server');

     $self->{output}->perfdata_add(label    => 'mailboxes',
                                   value    => $i);
...
Check number of mailboxes from your Bluemind server.
