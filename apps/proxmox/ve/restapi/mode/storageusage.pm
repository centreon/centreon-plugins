...
sub custom_storage_perfdata {
  ...
  $self->{output}->perfdata_add(label => 'storage_used' . $extra_label, unit => 'B',
...
sub set_counters {
  ...
  { name => 'storages', type => 1, cb_prefix_output => 'prefix_storages_output', message_multiple => 'All storages are ok', skipped_code => { -11 => 1 } },
  ];
...
sub manage_selection {
  ...
  my $result = $options{custom}->api_get_storages(storage_id => $self->{option_results}->{storage_id},
  storage_name => $self->{option_results}->{storage_name}, statefile => $self->{statefile_cache_storages});
