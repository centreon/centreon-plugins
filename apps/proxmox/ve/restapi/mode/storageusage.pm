...
sub custom_status_threshold {
    ...
    eval {
        ...
        if (
            defined(
                $instance_mode->{option_results}->{critical_storage_status}
            )
            && $instance_mode->{option_results}->{critical_storage_status} ne ''
            && eval
            "$instance_mode->{option_results}->{critical_storage_status}"
          )
        {
            ...
            defined(
                $instance_mode->{option_results}->{warning_storage_status}
            )
            && $instance_mode->{option_results}->{warning_storage_status} ne ''
...
sub custom_storage_perfdata {
    ...
    $self->{output}->perfdata_add(
        label   => 'storage_used' . $extra_label,
        unit    => 'B',
...
sub custom_storage_calc {
    ...
    $self->{result_values}->{total} =
      $options{new_datas}->{ $self->{instance} . '_storage_total' };
    $self->{result_values}->{used} =
      $options{new_datas}->{ $self->{instance} . '_storage_usage' };
    ...
    $self->{maps_counters_type} = [
        {
            name             => 'storages',
            type             => 1,
            cb_prefix_output => 'prefix_storages_output',
        ...
        },
        {
            ...
            set   => {
                ...
                closure_custom_perfdata =>
                  $self->can('custom_storage_perfdata'),
                closure_custom_threshold_check =>
                  $self->can('custom_storage_threshold'),
            }
        },
...
sub new {
    ...
    $options{options}->add_options(
        arguments => {
            ...
            "warning-storage-status:s" =>
              { name => 'warning_storage_status', default => '' },
            "critical-storage-status:s" =>
              { name => 'critical_storage_status', default => '' },
        }
    );
    $self->{statefile_cache_storages} =
...
sub manage_selection {
    ...
    $self->{cache_name} =
      ...
      . '_'
      . (
        defined( $self->{option_results}->{storage_id} )
        ? md5_hex( $self->{option_results}->{storage_id} )
        : md5_hex('all') )
      . '_'
      . (
        defined( $self->{option_results}->{storage_name} )
        ? md5_hex( $self->{option_results}->{storage_name} )
        : md5_hex('all') );
}
...
Check storage usage on Proxmox VE Cluster.
