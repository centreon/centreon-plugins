...
sub custom_status_threshold {
    ...
    eval {
        ...
        if ( defined( $instance_mode->{option_results}->{critical_node_status} )
            && $instance_mode->{option_results}->{critical_node_status} ne ''
            && eval "$instance_mode->{option_results}->{critical_node_status}" )
        {
            $status = 'critical';
        }
        elsif (
            defined( $instance_mode->{option_results}->{warning_node_status} )
            && $instance_mode->{option_results}->{warning_node_status} ne ''
...
sub custom_swap_calc {
    ...
    $self->{result_values}->{display} =
      $options{new_datas}->{ $self->{instance} . '_display' };
    $self->{result_values}->{total} =
      $options{new_datas}->{ $self->{instance} . '_swap_total' };
    $self->{result_values}->{used} =
      $options{new_datas}->{ $self->{instance} . '_swap_usage' };
    $self->{result_values}->{free} =
      $self->{result_values}->{total} - $self->{result_values}->{used};
    $self->{result_values}->{prct_free} =
      $self->{result_values}->{free} * 100 / $self->{result_values}->{total};
...
sub custom_fs_calc {
    ...
    $self->{result_values}->{display} =
      $options{new_datas}->{ $self->{instance} . '_display' };
    $self->{result_values}->{total} =
      $options{new_datas}->{ $self->{instance} . '_fs_total' };
    $self->{result_values}->{used} =
      $options{new_datas}->{ $self->{instance} . '_fs_usage' };
    $self->{result_values}->{free} =
      $self->{result_values}->{total} - $self->{result_values}->{used};
    $self->{result_values}->{prct_free} =
      $self->{result_values}->{free} * 100 / $self->{result_values}->{total};
    ...
    $self->{maps_counters_type} = [
        {
            name             => 'nodes',
            type             => 1,
            cb_prefix_output => 'prefix_nodes_output',
    ...
    $self->{maps_counters}->{nodes} = [
...

        {
            ...
            set   => {
                ...
                closure_custom_threshold_check =>
                  $self->can('custom_swap_threshold'),
            }
        },
...
sub new {
    ...
    $options{options}->add_options(
        arguments => {
            ...
            "warning-node-status:s" =>
              { name => 'warning_node_status', default => '' },
            "critical-node-status:s" =>
              { name => 'critical_node_status', default => '' },
        }
    );
...
sub prefix_nodes_traffic_output {
    my ( $self, %options ) = @_;
...
sub manage_selection {
    ...
    $self->{cache_name} =
      ...
      . '_'
      . (
        defined( $self->{option_results}->{node_id} )
        ? md5_hex( $self->{option_results}->{node_id} )
        : md5_hex('all') )
      . '_'
      . (
        defined( $self->{option_results}->{node_name} )
        ? md5_hex( $self->{option_results}->{node_name} )
        : md5_hex('all') );
}
...
Check node usage on Proxmox VE Cluster.
