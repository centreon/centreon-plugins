...
sub custom_status_threshold {
    ...
    eval {
        ...
        if (   defined( $instance_mode->{option_results}->{critical_vm_status} )
            && $instance_mode->{option_results}->{critical_vm_status} ne ''
            && eval "$instance_mode->{option_results}->{critical_vm_status}" )
        {
            $status = 'critical';
        }
        elsif (defined( $instance_mode->{option_results}->{warning_vm_status} )
            && $instance_mode->{option_results}->{warning_vm_status} ne ''
    ...
    if ( defined($message) ) {
        $self->{output}
          ->output_add( long_msg => 'filter status issue: ' . $message );
    }
...
sub custom_status_calc {
    ...
    $self->{result_values}->{state} =
      $options{new_datas}->{ $self->{instance} . '_state' };
    $self->{result_values}->{name} =
      $options{new_datas}->{ $self->{instance} . '_name' };
...
sub custom_cpu_calc {
    ...
    my $delta_cpu_total =
      $options{new_datas}->{ $self->{instance} . '_cpu_total_usage' } -
      $options{old_datas}->{ $self->{instance} . '_cpu_total_usage' };
...
sub custom_memory_perfdata {
    ...
    $self->{output}->perfdata_add(
        label   => 'memory_used' . $extra_label,
        unit    => 'B',
        value   => $self->{result_values}->{used},
        warning => $self->{perfdata}->get_perfdata_for_output(
            label    => 'warning-' . $self->{label},
            total    => $self->{result_values}->{total},
            cast_int => 1
        ),
        critical => $self->{perfdata}->get_perfdata_for_output(
            label    => 'critical-' . $self->{label},
            total    => $self->{result_values}->{total},
            cast_int => 1
        ),
        min => 0,
...
sub custom_memory_output {
    ...
    my ( $total_size_value, $total_size_unit ) = $self->{perfdata}
      ->change_bytes( value => $self->{result_values}->{total} );
    my ( $total_used_value, $total_used_unit ) = $self->{perfdata}
      ->change_bytes( value => $self->{result_values}->{used} );
    my ( $total_free_value, $total_free_unit ) = $self->{perfdata}
      ->change_bytes( value => $self->{result_values}->{free} );
...
sub custom_memory_calc {
    ...
    $self->{result_values}->{total} =
      $options{new_datas}->{ $self->{instance} . '_memory_total' };
    $self->{result_values}->{used} =
      $options{new_datas}->{ $self->{instance} . '_memory_usage' };
    $self->{result_values}->{free} =
      $self->{result_values}->{total} - $self->{result_values}->{used};
    $self->{result_values}->{prct_free} =
      $self->{result_values}->{free} * 100 / $self->{result_values}->{total};
    $self->{result_values}->{prct_used} =
      $self->{result_values}->{used} * 100 / $self->{result_values}->{total};
    return 0;
}
    ...
    $self->{maps_counters_type} = [
        {
            name             => 'vms',
            type             => 1,
            cb_prefix_output => 'prefix_vms_output',
        ...
        },
        {
            name             => 'vms_traffic',
            type             => 1,
            cb_prefix_output => 'prefix_vms_traffic_output',
            message_multiple => 'All vm traffics are ok',
            skipped_code     => { -11 => 1 }
        },
    ];
    ...
    $self->{maps_counters}->{vms} = [
        {
            ...
            set       => {
                ...
                closure_custom_threshold_check =>
                  $self->can('custom_status_threshold'),
            }
        },
        {
            label => 'cpu',
        ...
        },
        {
            ...
            set   => {
                key_values =>
                  [ { name => 'read_io', diff => 1 }, { name => 'display' } ],
                per_second            => 1,
                output_template       => 'Read IOPs : %.2f',
                output_error_template => "Read IOPs : %s",
                perfdatas             => [
                    {
                        label                => 'read_iops',
                        value                => 'read_io_per_second',
                        template             => '%.2f',
                        unit                 => 'iops',
                        min                  => 0,
                        label_extra_instance => 1,
        ...
        },
        {
            ...
            set   => {
                key_values =>
                  [ { name => 'write_io', diff => 1 }, { name => 'display' } ],
                per_second            => 1,
                output_template       => 'Write IOPs : %.2f',
                output_error_template => "Write IOPs : %s",
                perfdatas             => [
                    {
                        label                => 'write_iops',
                        value                => 'write_io_per_second',
                        template             => '%.2f',
                        unit                 => 'iops',
                        min                  => 0,
                        label_extra_instance => 1,
    ...
    $self->{maps_counters}->{vms_traffic} = [
        {
            ...
            set   => {
                key_values => [
                    { name => 'traffic_in', diff => 1 }, { name => 'display' }
                ],
                per_second          => 1,
...
sub new {
    my ( $class, %options ) = @_;
    my $self =
      $class->SUPER::new( package => __PACKAGE__, %options, statefile => 1 );
    bless $self, $class;
    ...
    $options{options}->add_options(
        arguments => {
            ...
            "warning-vm-status:s" =>
              { name => 'warning_vm_status', default => '' },
            "critical-vm-status:s" =>
              { name => 'critical_vm_status', default => '' },
        }
    );
...
sub change_macros {
    ...
    foreach ( ( 'warning_vm_status', 'critical_vm_status' ) ) {
        if ( defined( $self->{option_results}->{$_} ) ) {
            $self->{option_results}->{$_} =~
              s/%\{(.*?)\}/\$self->{result_values}->{$1}/g;
        }
    }
}
...
sub manage_selection {
    ...
    foreach my $vm_id ( keys %{$result} ) {
            ...
            && $name !~ /$self->{option_results}->{filter_name}/ )
        {
            $self->{output}->output_add(
                long_msg => "skipping  '" . $name . "': no matching filter.",
                debug    => 1
            );
            next;
        }
    ...
    $self->{cache_name} =
      ...
      . '_'
      . (
        defined( $self->{option_results}->{vm_id} )
        ? md5_hex( $self->{option_results}->{vm_id} )
        : md5_hex('all') )
      . '_'
      . (
        defined( $self->{option_results}->{vm_name} )
        ? md5_hex( $self->{option_results}->{vm_name} )
        : md5_hex('all') );
}
