...
sub new {
    ...
    $options{options}->add_options(
        arguments => {
            ...
            "reload-cache-time:s" =>
              { name => 'reload_cache_time', default => 7200 },
        }
    );
...
sub set_defaults {
    ...
    foreach ( keys %{ $options{default} } ) {
        if ( $_ eq $self->{mode} ) {
            for (
                my $i = 0 ;
                $i < scalar( @{ $options{default}->{$_} } ) ;
                $i++
              )
            {
                foreach my $opt ( keys %{ $options{default}->{$_}[$i] } ) {
                    if ( !defined( $self->{option_results}->{$opt}[$i] ) ) {
                        $self->{option_results}->{$opt}[$i] =
                          $options{default}->{$_}[$i]->{$opt};
                    }
                }
            }
        }
...
sub check_options {
    ...
    $self->{hostname} =
      ( defined( $self->{option_results}->{hostname} ) )
      ? $self->{option_results}->{hostname}
      : undef;
    $self->{port} =
      ( defined( $self->{option_results}->{port} ) )
      ? $self->{option_results}->{port}
      : 8006;
    $self->{proto} =
      ( defined( $self->{option_results}->{proto} ) )
      ? $self->{option_results}->{proto}
      : 'https';
    $self->{api_username} =
      ( defined( $self->{option_results}->{api_username} ) )
      ? $self->{option_results}->{api_username}
      : undef;
    $self->{api_password} =
      ( defined( $self->{option_results}->{api_password} ) )
      ? $self->{option_results}->{api_password}
      : undef;
    $self->{realm} =
      ( defined( $self->{option_results}->{realm} ) )
      ? $self->{option_results}->{realm}
      : 'pam';

    if ( !defined( $self->{hostname} ) ) {
        $self->{output}
          ->add_option_msg( short_msg => "Need to specify hostname option." );
        $self->{output}->option_exit();
    }

    if ( !defined( $self->{api_username} ) ) {
        $self->{output}
          ->add_option_msg( short_msg => "Need to specify username option." );
        $self->{output}->option_exit();
    }

    if ( !defined( $self->{api_password} ) ) {
        $self->{output}
          ->add_option_msg( short_msg => "Need to specify password option." );
        $self->{output}->option_exit();
    }
...
sub settings {
    ...
    if ( !defined( $self->{proxmox_ticket} ) ) {
        $self->{proxmox_ticket} =
          $self->get_ticket( statefile => $self->{cache} );
    }
    $self->{http}->add_header(
        key   => 'Cookie',
        value => 'PVEAuthCookie=' . $self->{proxmox_ticket}
    );
    $self->{http}
...
sub get_ticket {
    my ( $self, %options ) = @_;
    my $has_cache_file =
      $options{statefile}->read( statefile => 'proxmox_ve_api_'
          . md5_hex( $self->{option_results}->{hostname} ) . '_'
          . md5_hex( $self->{option_results}->{api_username} ) );
    ...
    if ( $renew_ticket == 1 ) {
        ...
        if ($@) {
            ...
            $self->{output}->add_option_msg(
                short_msg => "Cannot decode json response for Token" );
            $self->{output}->option_exit();
        }
...
sub request_api {
    ...
    if ($@) {
        $self->{output}->output_add( long_msg => $content, debug => 1 );
        $self->{output}
          ->add_option_msg( short_msg => "Cannot decode json response: $@" );
        $self->{output}->option_exit();
    }
...
sub get_version {
    ...
    $self->settings();
    my $content =
      $self->request_api( method => 'GET', url_path => '/api2/json/version' );
...
sub internal_api_list_vms {
    my ( $self, %options ) = @_;
    $self->settings();
    my $vm = $self->request_api(
        method   => 'GET',
        url_path => '/api2/json/cluster/resources?type=vm'
    );
    if ($@) {
        $vm = [];
...
sub internal_api_list_nodes {
    my ( $self, %options ) = @_;
    $self->settings();
    my $nodes = $self->request_api(
        method   => 'GET',
        url_path => '/api2/json/cluster/resources?type=node'
    );
    if ($@) {
        $nodes = [];
        $self->{output}->output_add(
...
sub internal_api_list_storages {
    my ( $self, %options ) = @_;
    $self->settings();
    my $storage = $self->request_api(
        method   => 'GET',
        url_path => '/api2/json/cluster/resources?type=storage'
    );
    if ($@) {
        $storage = [];
        $self->{output}->output_add(
...
sub cache_vms {
    my ( $self, %options ) = @_;
    my $has_cache_file =
      $options{statefile}->read( statefile => 'cache_proxmox_vm_'
          . $self->{hostname} . '_'
          . $self->{port} );
    my $timestamp_cache = $options{statefile}->get( name => 'last_timestamp' );
    my $vms             = $options{statefile}->get( name => 'vms' );
    if (   $has_cache_file == 0
        || !defined($timestamp_cache)
        || ( ( time() - $timestamp_cache ) >
            ( ( $options{reload_cache_time} ) ) ) )
    {
        $vms = {};
...
sub cache_nodes {
    my ( $self, %options ) = @_;
    my $has_cache_file =
      $options{statefile}->read( statefile => 'cache_proxmox_node_'
          . $self->{hostname} . '_'
          . $self->{port} );
    my $timestamp_cache = $options{statefile}->get( name => 'last_timestamp' );
...
sub cache_storages {
    my ( $self, %options ) = @_;
    my $has_cache_file =
      $options{statefile}->read( statefile => 'cache_proxmox_storage_'
          . $self->{hostname} . '_'
          . $self->{port} );
    my $timestamp_cache = $options{statefile}->get( name => 'last_timestamp' );
...
sub internal_api_get_vm_stats {
    ...
    my $vm_stats;
    eval { $vm_stats = decode_json($response); };
    if ($@) {
        $vm_stats = {};
        $self->{output}->output_add(
            severity => 'UNKNOWN',
            short_msg =>
"Node '$options{node_id}': cannot decode json get vm '$options{vm_id}' stats response: $@"
        );
    }
...
sub internal_api_get_node_stats {
    ...
    my $node_stats;
    eval { $node_stats = decode_json($response); };
    if ($@) {
        $node_stats = {};
        $self->{output}->output_add(
            severity => 'UNKNOWN',
            short_msg =>
"Node '$options{node_id}': cannot decode json get stats response: $@"
        );
    }
...
sub internal_api_get_storage_stats {
    ...
    my $storage_stats;
    eval { $storage_stats = decode_json($response); };
    if ($@) {
        $storage_stats = {};
        $self->{output}->output_add(
            severity => 'UNKNOWN',
            short_msg =>
"Node '$options{storage_id}': cannot decode json get stats response: $@"
        );
    }
...
sub api_get_vms {
    ...
    if ( defined( $options{vm_id} ) && $options{vm_id} ne '' ) {
        if ( defined( $content_total->{ $options{vm_id} } ) ) {
            $content_total->{ $options{vm_id} }->{Stats} =
              $self->internal_api_get_vm_stats(
                node_id =>
                  $self->internal_api_get_vm_node( vm_id => $options{vm_id} ),
                vm_id => $options{vm_id}
              );
        }
    }
        ...
        if ( defined($vm_id) ) {
            $content_total->{$vm_id}->{Stats} =
              $self->internal_api_get_vm_stats(
                node_id => $content_total->{$vm_id}->{Node},
                vm_id   => $vm_id
              );
...
sub api_get_nodes {
    ...
    if ( defined( $options{node_id} ) && $options{node_id} ne '' ) {
        if ( defined( $content_total->{ $options{node_id} } ) ) {
            $content_total->{ $options{node_id} }->{Stats} =
              $self->internal_api_get_node_stats(
        ...
        if ( defined($node_id) ) {
            $content_total->{$node_id}->{Stats} =
              $self->internal_api_get_node_stats( node_id => $node_id );
...
sub api_get_storages {
    my ( $self, %options ) = @_;
    my $content_total =
      $self->cache_storages( statefile => $options{statefile} );

    if ( defined( $options{storage_id} ) && $options{storage_id} ne '' ) {
        if ( defined( $content_total->{ $options{storage_id} } ) ) {
            $content_total->{ $options{storage_id} }->{Stats} =
              $self->internal_api_get_storage_stats(
    ...
    elsif ( defined( $options{storage_name} ) && $options{storage_name} ne '' )
    {
        ...
        if ( defined($storage_id) ) {
            $content_total->{$storage_id}->{Stats} =
              $self->internal_api_get_storage_stats(
