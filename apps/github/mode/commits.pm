    ...
    $options{options}->add_options(arguments =>
        {
            ...
            "urlpath:s"         => { name => 'url_path', default => '/repos' },
...
sub run {
    ...
    $self->{statefile_value}->read(statefile => 'github_' . $self->{option_results}->{repository} . '_' . $self->{option_results}->{user} . '_' . centreon::plugins::httplib::get_port($self) . '_' . $self->{mode});
    ...
    $self->{option_results}->{url_path} = $self->{option_results}->{url_path}."/".$self->{option_results}->{user}."/".$self->{option_results}->{repository}."/commits";
...
Set path to get GitHub's API commit information (Default: '/repos')
