    ...
    $options{options}->add_options(arguments =>
        {
            ...
            "urlpath:s"         => { name => 'url_path', default => '/pulls' },
...
sub run {
    ...
    $self->{option_results}->{url_path} = $self->{option_results}->{url_path}."/".$self->{option_results}->{user}."/".$self->{option_results}->{repository}."/pulls";
    ...
    $self->{output}->output_add(severity => $exit,
                                short_msg => sprintf("%d pull requests are open", $nb_issues ));
...
Set path to get GitHub's api pull requests information (Default: '/pulls')
