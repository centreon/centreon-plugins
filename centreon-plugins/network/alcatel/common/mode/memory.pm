...
package network::alcatel::omniswitch::6850::mode::memory;
...
sub run {
    ...
    my $oid_healthDeviceMemory1MinAvg = '.1.3.6.1.4.1.6486.800.1.2.1.16.1.1.1.10.0';
    my $oid_healthDeviceMemory1HrAvg = '.1.3.6.1.4.1.6486.800.1.2.1.16.1.1.1.11.0';
    my $result = $self->{snmp}->get_table(oid => [$oid_healthDeviceMemory1MinAvg, $oid_healthDeviceMemory1HrAvg], nothing_quit => 1);
    
    my $mem1min = $result->{$oid_healthDeviceMemory1MinAvg};
    my $mem1hour = $result->{$oid_healthDeviceMemory1HrAvg};

    my $exit1 = $self->{perfdata}->threshold_check(value => $mem1min, threshold => [ { label => 'crit1m', 'exit_litteral' => 'critical' },
