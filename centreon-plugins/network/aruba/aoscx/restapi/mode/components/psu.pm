...
package network::aruba::aoscx::restapi::mode::components::psu;
...
sub check {
    ...
    foreach my $subsystem_name (keys %{$self->{subsystems}}) {
        foreach my $endpoint (@{$self->{subsystems}->{$subsystem_name}->{power_supplies}}) {
            ...
            $self->{output}->output_add(
                long_msg => sprintf(
                    "power supply '%s' subsystem '%s' status is %s [instance: %s]",
                    $entry->{name},
                    $subsystem_name,
                    $entry->{status},
                    $instance
            ...
            if (!$self->{output}->is_status(value => $exit, compare => 'ok', litteral => 1)) {
                $self->{output}->output_add(
                    severity =>  $exit,
                    short_msg => sprintf(
                        "power supply '%s' subsystem '%s' status is %s",
