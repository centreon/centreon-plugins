...
package hardware::devices::polycom::trio::restapi::custom::api;
...
sub request_api {
    ...
    if (!sysopen($fh, $lockfile, O_WRONLY | O_CREAT)) {
        $self->{output}->add_option_msg(short_msg => "cannot open/create lock file '" . $lockfile . "': $!");
        $self->{output}->option_exit();
    }
