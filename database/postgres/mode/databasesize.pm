...
sub new {
    ...
    $options{options}->add_options(arguments =>
                                { 
                                  ...
                                  "database-size:s"              => { name => 'database', }, # database name to check
                                });
...
sub run {
    ...
    my $query = sprintf("SELECT pg_database_size('%s') LIMIT 1;",$self->{option_results}->{database});
    ...
    if (defined($result)) {
        ...
        $self->{output}->output_add(severity => $exit_code,
                                    short_msg => sprintf('Database "%s" size is %s %s',$self->{option_results}->{database}, $value, $value_unit));
        
        $self->{output}->perfdata_add(label => $self->{option_results}->{database}, unit => 'B',
                                      value => $result,
        ...
        $self->{output}->output_add(severity => 'UNKNOWN',
                                    short_msg => sprintf('Database size "%s" is unknown ...', $self->{option_results}->{database}));
    }
...
=item B<--database-size>

The of the database to check.
