...
package blockchain::hyperledger::hlfpoller::mode::tracking;
...
sub set_counters {
    ...
    $self->{maps_counters}->{events} = [
       { label => 'events-frequency', nlabel => 'hyperledger.tracking.events.perminute', set => {
    ...
    $self->{maps_counters}->{mining} = [
       { label => 'mining-frequency', nlabel => 'hyperledger.tracking.mined.block.perminute', set => {
        ...
        { label => 'mining-prct', nlabel => 'hyperledger.tracking.mined.block.prct', display_ok => 0, set => {
                key_values => [],
                manual_keys => 1,
...
Check hyperledger fabric poller for events, miners and balances tracking
