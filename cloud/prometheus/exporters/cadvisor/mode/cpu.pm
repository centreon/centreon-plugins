...
sub check_options {
    ...
    if ($self->{option_results}->{container} eq '') {
        $self->{output}->add_option_msg(short_msg => "Need to specify container option.");
        $self->{output}->option_exit();
    }
    if ($self->{option_results}->{pod} eq '') {
        $self->{output}->add_option_msg(short_msg => "Need to specify pod option.");
        $self->{output}->option_exit();
    }
...
sub manage_selection {
    ...
    my $results = $options{custom}->query_range(queries => [ 'label_replace((irate({__name__=~"' . $self->{metrics}->{usage} . '",' .
                                                                ...
                                                                $extra_filter . '}[1m])) * 100, "__name__", "usage", "", "")',
                                                             'label_replace((irate({__name__=~"' . $self->{metrics}->{throttled} . '",' .
                                                                $self->{option_results}->{container} . ',' .
                                                                $self->{option_results}->{pod} .
                                                                $extra_filter . '}[1m])) * 100, "__name__", "throttled", "", "")' ],
