...
sub new {
    ...
    $options{options}->add_options(arguments =>
                                {
                                  "warning:s"               => { name => 'warning', default => '20:'},
                                  "critical:s"              => { name => 'critical', default => '10:'},
...
sub run {
    ...
    my $account = $self->{option_results}->{account};
    ...
    my $Answer = $Api->get(path => '/sms/'.$account);
    ...
    my $smsLeft = $decoded->{'creditsLeft'};

    my $exit = $self->{perfdata}->threshold_check(value => $smsLeft,
    ...
    $self->{output}->perfdata_add(label => "left", unit => 'sms',
                                  value => $smsLeft,
                                  warning => $self->{perfdata}->get_perfdata_for_output(label => 'warning', left => $self->{option_results}->{warning}, cast_int => 1),
                                  critical => $self->{perfdata}->get_perfdata_for_output(label => 'critical', left => $self->{option_results}->{critical}, cast_int => 1),
                                  min => 0);
