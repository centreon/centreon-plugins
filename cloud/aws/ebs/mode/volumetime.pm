...
package cloud::aws::ebs::mode::volumetime;
...
sub custom_metric_calc {
    ...
    $self->{result_values}->{period} = $options{new_datas}->{$self->{instance} . '_period'};
...
sub custom_metric_output {
    ...
    if (defined($metrics_mapping{$self->{result_values}->{metric}}->{display_percent})) {
         my $percent = ($self->{result_values}->{value} / $self->{result_values}->{period} * 100);
         $msg = sprintf("%s: %.2f%s (%.2f%)", $metrics_mapping{$self->{result_values}->{metric}}->{output}, $value, $unit, $percent);
    } else {
...
sub set_counters {
    ...
    foreach my $metric (keys %metrics_mapping) {
        my $entry = {
            ...
            set => {
                key_values                          => [ { name => $metric }, { name => 'timeframe' }, { name => 'display' }, { name => 'period'} ],
    ...
    foreach my $instance (@{$self->{aws_instance}}) {
        ...
        foreach my $metric (@{$self->{aws_metrics}}) {
            foreach my $statistic (@{$self->{aws_statistics}}) {
                ...
                $self->{metrics}->{$instance}->{statistics}->{lc($statistic)}->{period} = $self->{aws_period};
...
Check Amazon Elastic Block Store volumes operations time.

Example:
perl centreon_plugins.pl --plugin=cloud::aws::ebs::plugin --custommode=awscli --mode=volumetime --region='eu-west-1'
...
Can be: VolumeTotalReadTime, VolumeTotalWriteTime, VolumeIdleTime

=item B<--warning-$metric$>

Warning thresholds ($metric$ can be: 'total-read-time', 'total-write-time', 'idle-time').

=item B<--critical-$metric$>

Critical thresholds ($metric$ can be: 'total-read-time', 'total-write-time', 'idle-time').
