...
package cloud::azure::network::appgateway::mode::backendstatus;
...
sub get_metrics_mapping {
    ...
    my $metrics_mapping = {
        'backendresponsestatus' => {
            'output' => 'Backend Response Status',
            'label'  => 'response-status',
            'nlabel' => 'appgateway.backend.response.status.count',
            'unit'   => '',
            'min'    => '0'
        }
    };
...
Check Azure Application Gateway backend response status.
...
--aggregation='total' --warning-response-status='1000' --critical-response-status='2000'
...
=item B<--warning-response-status>

Warning threshold.

=item B<--critical-response-status>
