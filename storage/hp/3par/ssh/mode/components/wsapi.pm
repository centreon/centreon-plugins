...
sub check {
    ...
    foreach (@results) {
        ...
        $self->{output}->output_add(
            long_msg => sprintf(
                "wsapi service state is '%s' [status: '%s'] [HTTP on port %d is %s] [HTTPS on port %d is %s] [instance: %s]",
        ...
        if (!$self->{output}->is_status(value => $exit, compare => 'ok', litteral => 1)) {
            $self->{output}->output_add(
                severity =>  $exit,
                short_msg => sprintf(
                    "wsapi service state is '%s' [instance: %s]",
        ...
        if (!$self->{output}->is_status(value => $exit, compare => 'ok', litteral => 1)) {
            $self->{output}->output_add(
                severity =>  $exit,
                short_msg => sprintf(
                    "wsapi service status is '%s' [instance: %s]",
