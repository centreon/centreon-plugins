...
package storage::emc::xtremio::restapi::mode::dataprotectiongroups;
...
sub run {
    ...
    $self->{output}->output_add(severity => 'OK',
                                short_msg => 'All DPG are OK');

    foreach my $item (@items) {
        next if ($self->check_filter(section => 'dpg', instance => $item));
...
Check DPG health indicators ('dpg-state', 'protection-state', 'rebuild-in-progress', 'ssd-preparation-in-progress', 'rebalance-progress');
...
Can also exclude specific instance: --filter=dpg,DPG-NAME

=item B<--threshold-overload>

Overload a specific threshold, e.g --threshold-overload="protection-state,CRITICAL,normal"
