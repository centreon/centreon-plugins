...
package os::windows::wmi::mode::storage;
...
sub manage_selection {
    ...
    my $WQL = 'Select Capacity,DeviceID,DriveLetter,DriveType,FileSystem,FreeSpace,Label,Name from Win32_Volume where DriveType=' . $self->default_storage_type ;
...
#   my @results = split /\n/, $result;
    ...
    $self->{storage} = {};
    while ($result =~ /^(\d+);.*?;(.*?);(.*?);(.*?);(.*?);.*?;.*?$/msg) {
        my ($size, $name_storage, $type, $fileSystem, $free) = ($1, $2, $3, $4, $5);
        my $used = $size - $free;
...
Filter storage types (Default: '3').
