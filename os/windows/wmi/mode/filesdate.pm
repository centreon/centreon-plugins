...
package os::windows::wmi::mode::filesdate;
...
sub run {
    ...
    my $WQL = 'Select name,lastmodified from CIM_DataFile where name = "' . $self->{option_results}->{file} . '"';
    ...
    while ($result =~ /^(\d+.*?);(.*?)$/msg) {
        ...
        $self->{output}->output_add(long_msg => sprintf("%s: %s seconds (time: %s)", $name, $diff_time, scalar(localtime($time))));
            $self->{output}->output_add(
                severity => $exit_code,
                short_msg => sprintf('File %s: %s seconds (time: %s)', $name, $diff_time, scalar(localtime($time)))
            );
