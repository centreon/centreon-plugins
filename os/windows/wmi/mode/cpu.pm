...
package os::windows::wmi::mode::cpu;
...
sub manage_selection {
    ...
    my $WQL = 'select Name,PercentProcessorTime,Timestamp_Sys100NS from Win32_PerfRawData_PerfOS_Processor where Name != "_Total"';
    ...
    #0;258771562500;132846743493647620
    #
    ...
    while ($result =~ /^(\d+);(.*?);(.*?)$/msg) {
       ...
       $self->{cpu_core}->{$cpu_number} = {
           ...
           Timestamp_Sys100NS => $3
       };
       $self->{cpu_avg}->{'cpu' . $cpu_number . '_PercentProcessorTime'} = $2;
       $self->{cpu_avg}->{'cpu' . $cpu_number . '_Timestamp_Sys100NS'} = $3;
    }

    $self->{cache_name} = 'cache_windows_wmi_' . $options{custom}->get_identifier()  . '_' . $self->{mode} . '_' .
