...
package os::windows::wmi::mode::ntp;
...
sub get_target_time {
    ...
    my ($Day,$Hour,$Milliseconds,$Minute,$Month,$Second,$Year);
    while ($result =~ /^(\d+);\d+;(\d+);(\d+);(\d+);(\d+);\d+;(\d+);\d+;(\d+)$/msg) {
       ($Day,$Hour,$Milliseconds,$Minute,$Month,$Second,$Year) = ($1,$2,$3,$4,$5,$6,$7);
    }

    return $self->get_from_datetime(year   => $Year, 
                                    mounth => $Month, 
                                    day    => $Day, 
                                    hour   => $Hour, 
                                    minute => $Minute, 
