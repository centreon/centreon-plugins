...
package os::windows::wmi::mode::foldersize;
...
sub check_options {
    ...
    if (!defined($self->{option_results}->{drive}) || $self->{option_results}->{drive} eq '') {
       $self->{output}->add_option_msg(short_msg => "Need to specify drive option.");
       $self->{output}->option_exit();
    }
...
sub run {
    ...
    my $WQL = 'Select name,filesize from CIM_DataFile where drive = "' . $self->{option_results}->{drive} . '" AND path = "' . $self->{option_results}->{folder} . '"';
