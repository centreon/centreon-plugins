...
sub check_time {
    ...
    return $self->{perfdata}->threshold_check(value => $total_seconds_elapsed, threshold => [ { label => 'critical-time', 'exit_litteral' => 'critical' }, { label => 'warning-time', exit_litteral => 'warning' } ]);
}
...
sub run {
    ...
    my $num_processes_crit = 0;
    my $num_processes_warn = 0;
    ...
    foreach my $pid (keys %{$self->{result}}) {
        ...
        $self->{output}->output_add(long_msg => 'Process: [command => ' . $self->{result}->{$pid}->{cmd} . 
                                                          ...
                                                          '] [elapsed => '. $self->{result}->{$pid}->{elapsed} . ']');
        $crit = ($self->check_time(pid => $pid));

        $num_processes_crit++ if ($crit eq 'critical');
        $num_processes_warn++ if ($crit eq 'warning');
        $num_processes_match++;
    }
    ...
    if ($num_processes_warn > 0) {
        $self->{output}->output_add(severity => 'warning',
                                short_msg => sprintf("%s/%s processes have a warning time issue", $num_processes_warn, $num_processes_match));
    }

    if ($num_processes_crit > 0) {
        $self->{output}->output_add(severity => 'critical',
                                short_msg => sprintf("%s/%s processes have a critical time issue", $num_processes_crit, $num_processes_match));
    }
