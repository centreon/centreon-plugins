...
package os::linux::debian::local::mode::pendingupgrade;
    ...
    $options{options}->add_options(arguments =>
                                {
                                  ...
                                  "sudo"              => { name => 'sudo' },
                                  "command:s"         => { name => 'command', default => 'apt-get' },
                                  "command-path:s"    => { name => 'command_path' },
                                  "command-options:s" => { name => 'command_options', default => 'upgrade -s --dry-run 2>&1' },
...
sub manage_selection {
    ...
    foreach my $line (@lines) {
        ...
        my ($package, $version, $upgradePath, $baseOS) = ($2, $3, $5, $6);
...
Command to get information (Default: 'apt-get').
...
Command options (Default: 'upgrade -s --dry-run 2>&1').

=item B<--filter-name>

Filter update name (regexp can be used) eg: security to show only security updates.
