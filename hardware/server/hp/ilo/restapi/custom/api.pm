...
package hardware::server::hp::ilo::restapi::custom::api;
...
sub authenticate {
    ...
    my $has_cache_file = $options{statefile}->read(statefile => 'ilo_redfish_' . md5_hex($self->{option_results}->{hostname}) . '_' . md5_hex($self->{option_results}->{api_username}));
