...
package hardware::server::dell::ome_modular::snmp::mode::components::temperature;
...
sub load {
    my ($self) = @_;
    push @{$self->{request}}, {oid => $oid_dmmStatusNowGroup, start => $mapping->{dmmChassisFrontPanelAmbientTemperature}->{oid}, end => $mapping->{dmmChassisFrontPanelAmbientTemperature}->{oid}};
}
...
sub check
{
    ...
    foreach my $probe (keys %{$mapping}) {
        ...
        $self->{output}->output_add(
            long_msg => sprintf( "%s is %dC [instance: %s].",
