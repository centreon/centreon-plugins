...
package hardware::pdu::schleifenbauer::gtw::snmp::mode::pdustatus;
...
sub run {
  ...
  my $oid_status = ".1.3.6.1.4.1.31034.1.1.3.1.3.$Id.1";
  my ($result, $value);

  $result = $self->{snmp}->get_leef(oids => [ $oid_status ], nothing_quit => 1);
  $value = $result->{$oid_status};

  my $exit_code = $self->{perfdata}->threshold_check(value => $value,
                                                     threshold => [ { label => 'critical', 'exit_litteral' => 'critical' }, { label => 'warning', exit_litteral => 'warning' } ]);

  $self->{output}->perfdata_add(label => "PDU Status",
  ...
  $self->{output}->output_add(severity => $exit_code,
                              short_msg => sprintf("PDU Status : %.2f",
                                  $value));
