...
sub set_counters {
    ...
    $self->{maps_counters}->{global} = [
        { label => 'mains-frequency', nlabel => 'mains.frequency.hertz', set => {
                ...
                output_template => 'mains frequency: %.2f Hz',
                perfdatas => [
                    { value => 'mainsVL12Conm', template => '%.2f', min => 0, unit => 'Hz', },
                ],
            }
        },
        { label => 'genset-frequency', nlabel => 'genset.frequency.hertz', set => {
                key_values => [ { name => 'genFreqConm' } ],
                output_template => 'genset frequency: %.2f Hz',
                perfdatas => [
                    { value => 'genFreqConm', template => '%.2f', min => 0, unit => 'Hz', },
                ],
            }
        }
    ];
...
Check mains and genset frequency levels.

=over 8

=item B<--warning-*>

Warning threshold in Hertz.

Where '*' can be: 'mains-frequency', 'genset-frequency' 

=item B<--critical-*>

Critical threshold in Hertz.
