...
package network::stonesoft::mode::rateconnection;
...
sub run {
    ...
    if (defined($old_datas->{old_timestamp})) {
        ...
        my $current_absolute = $new_datas->{current_connections} - $old_datas->{old_current_connections};
        my $current_absolute_per_sec = $current_absolute / $time_delta;

        my $exit = $self->{perfdata}->threshold_check(value => $current_absolute_per_sec, threshold => [ { label => 'critical', 'exit_litteral' => 'critical' }, { label => 'warning', exit_litteral => 'warning' } ]);

        $self->{output}->output_add(severity => $exit,
                                    short_msg => sprintf("Rate Connection: %.2f /s",
                                                $current_absolute_per_sec));

        $self->{output}->perfdata_add(label => 'current_connections_per_sec',
                                    value => sprintf("%.2f", $current_absolute_per_sec),
...
Check current connections per second by firewall.

=over 8

=item B<--warning>

Threshold warning for current connections per second.

=item B<--critical>

Threshold critical for current connections per second.
