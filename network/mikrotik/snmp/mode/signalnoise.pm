...
package network::mikrotik::snmp::mode::signalnoise;
...
sub set_counters {
    ...
    $self->{maps_counters_type} = [
        { name => 'signalnoise', type => 1, cb_prefix_output => 'prefix_s2n_output', message_multiple => 'All signals are ok' }
    ];
...
sub manage_selection {
    ...
    my $oids = [$mapping->{'regmac'}, $mapping->{'regs2n'} ];
    ...
    foreach my $index (0 .. $#KeyMac) {
        ...
        next if ($Keys2n[$index] !~ /^$mapping->{'regs2n'}->{'oid'}\.(.*)$/);
        my $instance = $1;
        my $result = $options{snmp}->map_instance(mapping => $mapping, results => {$mapping->{'regs2n'}->{'oid'} => $interfaceTables->{ $mapping->{'regs2n'}->{'oid'} }}, 
                                                  instance => $instance);
    ...
    if (scalar(keys %{$self->{signalnoise}}) <= 0) {
