...
Disk-Io ${tc}
    ...
    Examples:    tc    extraoptions                              expected_result   --
        ...      1     ${EMPTY}                                  UNKNOWN: no data for host vm-1234 counter disk.throughput.usage.VM at the moment. - get_vm_stats function failed to retrieve stats get_vm_id_from_name method called to get web-server-02's id: vm-1234. Prefer using --vm-id to spare a query to the API. The counter disk.throughput.usage.VM was not recorded for resource vm-1234 before. It will now (creating acq_spec).
